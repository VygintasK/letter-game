    ...
    capsLatinLTLettersArr.map((letter)=>{
        ...
        oneLetter.addEventListener('click',()=>{
            ...
            if (randomLetter === letter){
                ...
                let screenBlockCont = myElementCreator('h1','screenBlockText','screenBlock')
                screenBlockCont.textContent = 'test element Timeout 4000'
                setTimeout(function() {
                    console.log('delete in 4000')
                    contentWrap.remove()
                    audioStorage.remove()
                    screenBlock.remove()
                    renderLetters()
...
test.addEventListener('click',()=>{
    let {width,height} = document.defaultView.window.screen
    console.log(height,'x',width)
    alert('Ekrano dydis:'+width+'x'+height)
})
